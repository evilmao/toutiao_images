...
client = pymongo.MongoClient(MONGO_URL, connect=False) #实例化一个monogodbd的客户端对象,开启多进程没执行一个多进程会实例化一个client对象
db = client[MONGO_DB]
    ...
    try:
        ...
        if data and 'data' in data.keys(): #如果返回的数据不为空，且字典键中包含'data'键
            for item in data.get('data'):# 获取data键对应的值--为一个列表
                yield item.get('article_url') #变量列表中的每一个元素都是一个字典，取出每个字典中键(article_url)对应的值，即每个图片主url
...
def save_image(content):
    ...
    '''
    file_path_name = '{0}/images/{1}.{2}'.format(os.getcwd(), md5(content).hexdigest(), 'jpg') #
